...
describe('Artist', function () {
  describe('.download', function () {
    it('downloads data from API and sets Artist instances', async function () {
      ...
      expect(Artist.find(1).name).to.eq('Niko')
      expect(Artist.find(2).name).to.eq('Lolita')
      expect(Artist.find(3).name).to.eq('キーヤキッス')
      expect(Artist.find(3).nameAlt).to.eq('KEY-A-KISS')
    })
  })
